    ...
    MySQL.Async.fetchAll(query, { random_id }, function(results)
        ...
        if VerifyPassword(src, password, result.password, true) then
            ...
            elseif Framework == 2 or Framework == 3 then
                ...
                exports["ox_inventory"]:RegisterStash(stash_id, "Container", container_info.type.slots, container_info.type.size)
                TriggerClientEvent("keep-containers:client:open", src, container_info.type, random_id)
            end
        end
    end)
end)
